...
**First, make sure you're on a version that includes the fix for this.** Two real bugs here were found and fixed after a real-cabinet report: `swap_video` didn't retry when `intro_video_target` was briefly locked (see below), and the generated `.bat`/`.vbs` always reported Task Scheduler's "Last Result" as `0` (success) even when the swap actually failed, which is why step 3 below couldn't be trusted before the fix. **If you enabled auto-run before upgrading, re-run `introvideo install-autorun --apply`** to regenerate the launcher files with the fix — the old ones don't self-update.
...
2. Run `spindoctor introvideo swap --apply` by hand (CLI, or the **Swap now** button) and confirm `intro_video_target` actually changes on disk (check the file's timestamp). If this works, the pool/target config is fine and the issue is specific to the scheduled run.
...
4. **Most common real-world cause, confirmed on an actual cabinet:** the logon task and HyperSpin's own launch (e.g. a `shell:startup` shortcut) both fire around login with no strict ordering guarantee, and HyperSpin can still be holding `intro_video_target` open — playing the *previous* intro video — at the exact moment the swap tries to overwrite it. `swap_video` now retries the copy for up to 10 seconds to ride this out; if the video *still* never changes after confirming steps 1-3, your intro clips may simply be longer than that retry window (`SWAP_RETRY_ATTEMPTS`/`SWAP_RETRY_DELAY_SECONDS` in `spindoctor/introvideo.py` — not currently exposed as a config option).
