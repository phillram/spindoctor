...
|---|---|
| `spindoctor` | Full CLI |
| `spindoctor-gui` | Tkinter GUI launcher with 15 tabs in workflow order: Setup · Audit & Doctor · Diagnose · Metadata & Media · Curate · Wheels · Main Menu · Systems · LEDBlinky · Lightgun · Backup & Restore · Tools · Migrate · Logs (per-run history) · Custom Command. On first launch (no `config.json` yet) the GUI auto-focuses the Setup tab; click **Run first-run wizard…** there for a guided 3-step setup, or reach the same dialog any time from **Help → First-run setup…**. Per-tab health badges show ⚠/✗ next to tabs whose area `doctor` flagged; Ctrl+F find bar over Output, Ctrl+Shift+F quick-filter narrowing every system combobox at once. Every tab scrolls vertically when content overflows. File / Help menus add shortcuts to config / logs (with click-to-undo) / HyperSpin theme browser / About / Keyboard shortcuts / Check for updates. See [GUI walkthrough](gui.md) for the full tab tour. |
