...
- **FE active animation** — `FELWAFile`: animation played while actively browsing HyperSpin. Leave blank for static colors; pick `<Random>` to have LedBlinky choose a different animation every time; pick a `.lwa` file for a smooth fade effect. Leave as `(leave unchanged)` — the default — to not touch this key at all.
- **Screen saver animation** — `FEScreenSaverLWAFile`: animation played during the HyperSpin screen saver. Leave blank to silence; pick `<Random>` to have LedBlinky choose a different animation every time; pick a `.lwa` file for a specific animation. Leave as `(leave unchanged)` — the default — to not touch this key at all.
...
`<Random>` is a literal value LedBlinky itself recognizes (it isn't a filename) — distinct from `(leave unchanged)`, which is purely a GUI convenience meaning "don't include this flag in the `patch-settings` call at all."
