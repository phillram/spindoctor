...
The **Videos** table (with a vertical scrollbar for longer pools) lists every video found either on disk (in `Random.ini`'s `Folder=` path, excluding its `Backup\` subfolder) or registered in `Random.ini`, with columns for on-disk presence, registration status (✓ / partial / -), and file size. On disk and Registered are independent — a video with on disk ✓ but Registered `-` has already been found in the folder (dropped in directly, restored from a backup, etc.) but isn't in the randomizer's rotation yet.

**↻ Refresh** re-scans. **Add video(s)** opens a multi-select file picker (defaulting to the randomizer's own video folder) to copy new file(s) in from elsewhere and register them — CLI: `introvideo add`. **Register selected** registers already on-disk row(s) as-is — no file picker, no copy — for videos found on disk but not yet in `FileList=`/`RandomList=`; it's the same `introvideo add` command, just pointed at the file's existing on-disk path instead of asking you to re-browse to a file that's already right there in the table. **Remove selected** drops the selected row(s) — Ctrl/Shift-click to select several — from `Random.ini`, without deleting the video files themselves — CLI: `introvideo remove`. **Shuffle order** randomizes the order every registered video is listed in — it never adds, removes, or deletes a video, only reorders the existing ones; useful because the third-party randomizer script may not itself vary its pick from one boot to the next, so pre-shuffling the list is what actually changes playback order across sessions — CLI: `introvideo shuffle`. None of the four action buttons pop a confirmation dialog; the global **Apply** checkbox is the gate — leaving it unticked runs a dry-run preview in the Output panel first, same as everywhere else in the GUI that shells out to a `--apply`-gated command. A multi-file add/register/remove writes a single shared `Random.ini` backup for the whole batch rather than one per file.
