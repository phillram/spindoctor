...
**Auto-run on Windows login** — a status label (Enabled / Disabled / "not available" on non-Windows) plus **Enable auto-run** / **Disable auto-run** buttons wrapping `introvideo install-autorun` / `uninstall-autorun`. Registers a Windows Task Scheduler logon task that runs a swap automatically at every login — no HyperSpin, RocketLauncher, or HyperHQ configuration involved, it's a plain login task that copies a file. The status label refreshes automatically after either button runs.
