...
**Fix game executable** (unnumbered): for games that launch the wrong executable (uninstaller, GOG/Steam cache file, NW.js runtime, wrong `.ahk` launcher, etc.). Works with any PCLauncher-backed system — not just PC Games. The system picker defaults to "PC Games" on startup (case-insensitive match); change it to "Taito Type X" or any other system to fix games in a system-level INI. The game dropdown lists all subdirectories found on disk — including games not yet in the HyperSpin XML — so it matches what `pc-rename` sees. The candidate listbox auto-populates with every `.exe` found in the game folder **and all subfolders** (e.g. both `J:\Games\PC Games\Coolio\coolio.exe` and `J:\Games\PC Games\Coolio\data\coolio.exe` are listed), recommended first — shallower paths rank above deeper ones; uninstallers, `vcredist*`, `chromedriver.exe`, NW.js runtime files, etc. are ranked last. For Taito Type X or other arcade-PC systems whose launcher is an `.ahk` script (not a `.exe`), use **Browse…** to locate the file directly — `.ahk` launchers do not appear in the auto-candidate list. Click **Apply** to update `Application=` and `WorkingFolder=` in the PCLauncher INI (per-game or system-level, detected automatically). Other user-set keys like `FadeTitle=` are left untouched. CLI: `spindoctor pc-fix-exe <system> <game> [--exe <path>] --apply`.
