...
**Step 3 — Fetch media:** Media-type checkboxes (wheel, background, snap, video, trailer, title, theme, fade, sound — defaulting to wheel + background), plus `--overwrite`. The **Source** dropdown (`screenscraper` / `thegamesdb` / config default) controls which provider is queried. By default both providers are tried and the best result merged: ScreenScraper fills a slot first; TheGamesDB fills any slot ScreenScraper missed. TheGamesDB provides clearlogos (→ wheel slot) and screenshots (→ snap slot) via its `Games/Images` endpoint — especially useful for newer indie PC games that have stub entries on ScreenScraper. All other types (video, title, fade, sound, theme) require ScreenScraper. See [commands.md → fetch-media provider capabilities](commands.md#fetch-media) for the full comparison table.
