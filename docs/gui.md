...
**Step 2 — Add a new system:** **Add Arcade System** (or **Add PC System** for a PC-games system) on a typed system name with optional **Skip system media download** / **Skip per-game media download** toggles. For PC systems the GUI automatically passes `--no-interactive` so the title-review step doesn't hang the subprocess on stdin — users who want to curate titles by hand can run `spindoctor add-pc-system <system>` from a terminal.
...
**Step 4 — Manage games in a system wheel:** Pick a system from the dropdown and click **Load Games** to populate a scrollable table showing every game in that system's XML (wheel) order — position number, ROM name, and display title. Reposition games with one of three methods: click **Move Up** / **Move Down**, press **Alt+↑** / **Alt+↓** to nudge without leaving the keyboard, or type a target position in the **Move to #** field and click **Go** to jump directly. **Sort A→Z (by title)** sorts by display name with leading articles stripped (The / A / An), matching HyperSpin's own wheel sort convention; **Sort A→Z (by ROM name)** sorts by filename. **Remove Game** shows a dry-run message when Apply is off; with Apply ticked it confirms and shells out to `game remove --apply` (the table row is removed only after the command reports success). Tick **Also remove PCLauncher INI** (checkbox next to the button) to also delete `Modules/PCLauncher/<system>/<game>.ini` — use this for PC systems so RocketLauncher no longer finds the game; the checkbox is a no-op for systems that have no PCLauncher INI. **Save Order** writes the full reordered list back to the system XML — dry-run preview when Apply is off, background-thread save when Apply is on; the save follows the same backup and atomic-write path as the Main Menu carousel. Changing the system dropdown clears the table so a loaded game list can't accidentally be saved to the wrong database. CLI: `spindoctor game list / remove / move / move-up / move-down / sort`.

**Step 5 — Organize a system (sort wheels + optional restructure):** Drives `spindoctor organize <SYSTEM>` with checkboxes for `--no-sort` and `--restructure`. Restructure honours the tab-level Apply toggle, plus a separate **Undo latest restructure** button for the `--undo` flow.

**Add new games / refresh a PC system** (unnumbered — for PC systems only): wraps `spindoctor add-pc-system <system> --no-menu --no-system-media --no-game-media --no-interactive`. Scans `<roms_dir>/<system>/` for new or changed installs, updates both the HyperSpin XML database (so new games appear in the wheel) and PCLauncher INIs. Run this after dropping new games into the folder.

The scanner enforces **one game per install folder**: all files inside a subfolder (e.g. `Peglin\`) are grouped together and a single best-candidate executable is chosen — so you never get duplicate entries like "Peglin" and "Peglin Launcher" from the same install. Website shortcut (`.url`) files left behind by some packages are automatically ignored, as are root-level "Launch X" shortcuts that duplicate a game's own subfolder.

Each INI is written with the actual game executable — when RocketLauncher's extension-matching picks up a non-exe file (e.g. `webcache.zip` from a GOG install), SpinDoctor scans the game folder and resolves the correct `.exe` automatically. With **Verbose** ticked, prints each game's resolved executable path and INI status (`new` / `stale` / `ok`) on separate lines with full paths — no truncation. **Overwrite existing INIs** rewrites every INI — use this after a drive migration, file rename, or to bulk-fix games previously written with the wrong executable. Running on a system that already has games is safe — existing XML entries are skipped, only new installs are added. CLI: `spindoctor add-pc-system <system>`.

**Fix game executable** (unnumbered): for games that launch the wrong executable (uninstaller, GOG/Steam cache file, NW.js runtime, wrong `.ahk` launcher, etc.). Works with any PCLauncher-backed system — not just PC Games. The system picker defaults to "PC Games" on startup (case-insensitive match); change it to "Taito Type X" or any other system to fix games in a system-level INI. The game dropdown lists all subdirectories found on disk — including games not yet in the HyperSpin XML — so it matches what `pc-rename` sees. The candidate listbox auto-populates with executables and launcher scripts found in the game folder **and all subfolders**, ranked in order: non-excluded `.exe` files first (shallower paths above deeper ones), then `.ahk` scripts, then `.bat` scripts, then excluded `.exe` files (uninstallers, `vcredist*`, `chromedriver.exe`, NW.js runtime files, etc.). For Taito Type X or other arcade-PC systems whose launcher is an `.ahk` script, the script appears in the candidate list — select it directly, or use **Browse…** to locate any file the picker can reach (accepts `.exe`, `.ahk`, and `.bat`). Click **Apply** to update `Application=` and `WorkingFolder=` in the PCLauncher INI (per-game or system-level, detected automatically). Other user-set keys like `FadeTitle=` are left untouched. CLI: `spindoctor pc-fix-exe <system> <game> [--exe <path>] --apply`.
...
- **Steam media** (optional, PC games only): Download trailer video(s), screenshots, header artwork, and/or a wheel image for a specific game directly from the Steam Store — no account or API key needed. Only useful for PC/Steam titles that ScreenScraper/TheGamesDB don't cover well. Workflow: (1) pick System + Game above; (2) paste a Steam store URL (`store.steampowered.com/app/1145360/Hades/`) or bare App ID into **Steam URL / App ID** — or click **Find** to auto-populate it (see below); (3) click **Scan** — SpinDoctor fetches the game's Steam page in the background and populates the **Video**, **Screenshot**, **Artwork**, and **Wheel** dropdowns with all available candidates; (4) pick the desired candidate from each dropdown — each dropdown also offers **"— do not download —"** as its first option; set any picker to that to skip that type (e.g. leave Wheel on "— do not download —" if you already have better wheel art from ScreenScraper); (5) click **Apply selected** to download only the selected types. Dropdowns show "— scan first —" until a scan completes, or "— none —" if Steam has nothing for that type. Each picker row has a **View** button — opens the selected candidate's direct URL in the browser for all types (images, video). For MP4 video candidates the browser will download or play inline; for HLS `.m3u8` candidates the browser downloads the manifest (use a separate player to view it). View buttons are disabled until a Scan has been run and reset when you switch game or system. Video candidates served via HLS (most 2024+ titles) are labelled `(HLS — needs ffmpeg)` in the dropdown — ffmpeg must be present next to `spindoctor.exe` or on `PATH` for these to download. The **Overwrite existing** checkbox controls whether an already-present file is replaced. The **Quality** dropdown (`Best (1080p)` / `720p` / `480p` / `360p`) controls the HLS resolution for video downloads — `Best (1080p)` picks the highest available quality (default); lower settings produce much smaller files (e.g. a 400 MB 1080p trailer becomes ~25 MB at 480p). Quality has no effect on MP4 or image candidates. Maps to `--hls-quality` on the CLI. A stored Steam App ID (saved via **Save override**) is pre-filled automatically when a game is loaded — so after saving once you can just click Scan without re-pasting the URL. Switching to a different game resets the URL field and all four pickers automatically. **Image format:** Steam serves wheel, artwork, and screenshot images as JPEG; SpinDoctor saves them as `.png` (HyperSpin requires PNG in those folders) and converts to real PNG when Pillow is installed. The **Wheel** slot uses Steam's header capsule image; it is not a transparent-logo equivalent but works as a placeholder when ScreenScraper has no wheel art. **Find (Steam App ID lookup):** Click **Find** (next to the Steam URL / App ID field) to search the Steam store by the selected game's name and auto-populate the field with the best match — no copy-pasting from a browser required. The search runs in the background; the status bar shows progress and the matched title + App ID when done. After Find populates the field, click **Scan** as usual to load available media candidates. If the auto-match is wrong (e.g. a DLC page instead of the base game), paste the correct URL manually. CLI equivalent: `spindoctor fetch-steam-media` — see [commands.md → fetch-steam-media](commands.md#fetch-steam-media).
