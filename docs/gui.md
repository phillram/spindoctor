...
The **Videos** table (with a vertical scrollbar for longer pools) lists every video found either on disk (in `Random.ini`'s `Folder=` path, excluding its `Backup\` subfolder) or registered in `Random.ini`, with columns for on-disk presence, registration status (✓ / partial / -), and file size. **↻ Refresh** re-scans. **Add video(s)…** opens a multi-select file picker; the chosen file(s) are copied into the randomizer's folder and registered — CLI: `introvideo add`. **Remove selected** drops the selected row(s) — Ctrl/Shift-click to select several — from `Random.ini` (after a confirmation dialog) without deleting the video files themselves — CLI: `introvideo remove`. Both buttons respect the global **Apply** checkbox; leaving it unticked runs a dry-run preview in the Output panel. A multi-file add/remove writes a single shared `Random.ini` backup for the whole batch rather than one per file.
