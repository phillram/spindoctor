...
- **1c. Sync player colors** — adds missing P2+/P3+ entries to `Colors.ini` by mirroring the matching P1 color for each button listed in `controls.ini`. `generate` only writes P1 keys; this step ensures multi-player games light all player buttons with the correct game-specific color rather than the XML fallback. Only adds keys declared in `controls.ini` — never overwrites existing entries. CLI: `spindoctor ledblinky colors sync-players`.
