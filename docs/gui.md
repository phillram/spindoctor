...
**Auto-refresh on cabinet startup** (Windows-only): a status label (Enabled / Disabled / "not available" on non-Windows) is always visible, no click needed — refreshes automatically after any of the three buttons below run, matching the Intro Video tab's Auto-run status label. If the task is registered but its generated `.bat` no longer matches the currently-running SpinDoctor install (typically after upgrading into a new version folder without re-clicking **Schedule auto-refresh**), the label says so explicitly in amber — "Outdated: Schedule auto-refresh to fix" — rather than a misleading plain "enabled"; re-clicking **Schedule auto-refresh** fixes it in place. **Schedule auto-refresh** registers a Task Scheduler `ONLOGON` task with a configurable post-log-on delay (default 2 min). **Remove scheduled task** and **Check task status** round out the lifecycle (the latter also writes a timestamped entry to the Output panel/Logs tab, useful for an audit trail beyond what the status label shows).
