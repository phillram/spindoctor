...
**↻ Refresh** re-scans. **Add video(s)…** opens a multi-select file picker to copy new file(s) in from elsewhere and register them — CLI: `introvideo add`. **Register selected** registers already on-disk row(s) as-is — no file picker, no copy — for videos found on disk but not yet in `FileList=`/`RandomList=`; it's the same `introvideo add` command, just pointed at the file's existing on-disk path instead of asking you to re-browse to a file that's already right there in the table. **Remove selected** drops the selected row(s) — Ctrl/Shift-click to select several — from `Random.ini` (after a confirmation dialog) without deleting the video files themselves — CLI: `introvideo remove`. All three action buttons respect the global **Apply** checkbox; leaving it unticked runs a dry-run preview in the Output panel. A multi-file add/register/remove writes a single shared `Random.ini` backup for the whole batch rather than one per file.
