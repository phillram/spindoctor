...
These three steps are MAME-specific — they use `mame -listxml` as the authoritative data source. Normal workflow: run **1a**, then **1c**. Run **1b** only if you have a legacy Colors.ini pre-dating SpinDoctor 2.4.21.

- **1a. Generate (controls + colors)** — reads MAME's `-listxml` data and writes `controls.ini` + `colors.ini` with per-game button assignments for every MAME ROM on your cabinet. Dry-run by default; preserves community-maintained entries (tick **Overwrite existing entries — 1a Generate** to replace them). Since 2.4.21, Colors.ini is written in LedBlinky's native named format (`P1_BUTTON1=Red`). Since 2.4.22, controls.ini is written with LedBlinky runtime key names (`P1_BUTTON1=1`) — if you have a controls.ini generated by 2.4.21 or earlier, tick **Overwrite existing entries** and run Generate once to fix it. CLI: `spindoctor ledblinky generate`.
- **1b. Normalize Colors.ini** — only needed if you have an existing Colors.ini in the legacy hex format (`ledcolor1=FF0000`, `joystick=FFFFFF`). Converts those entries to LedBlinky's native named format (`P1_BUTTON1=Red`, `P1_JOYSTICK=White`). Sections already in named format are untouched. Not required after a fresh Generate. CLI: `spindoctor ledblinky colors normalize`.
...
**Step 2 — Fill Default Colors (any console)**
