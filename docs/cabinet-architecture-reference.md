...
For a whole **library** of ready-made spatial/animated effects (sweeps, radial pulses, rain, cyclones, heartbeats, ripples, plasma, twinkles, a solid-colour breathe library, etc.), run **`python scripts/generate_lwax_patterns.py`** — a deterministic, no-AI batch generator that writes ~170 named `.lwax` files plus a `README.md` into `~/Downloads/spindoctor-lwax-patterns/`. It uses this cabinet's layout groups and the same builders in `spindoctor/lwax.py`; the same manual signing step applies to each file. See `docs/commands.md` → "Generating a full pattern batch" for the full walkthrough, and `.claude/skills/lwax-animation/SKILL.md` for the effect-authoring internals.
