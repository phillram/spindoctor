...
**Steam video URL format:** older Steam games include `movies[].mp4.max` — a direct MP4 URL that SpinDoctor downloads with a plain HTTP GET. Newer games (roughly 2024+) serve only HLS (`.m3u8`) and DASH (`.mpd`) streaming manifests with no direct MP4 link. `_parse_steam` prefers `mp4.max` (and `mp4.480` as a lower-quality fallback); if neither is present it records the `hls_h264` URL with `format="m3u8"`. `MediaDownloader._download_to` detects `.m3u8` URLs and delegates to `_download_hls`, which runs `ffmpeg -i <url> -c copy -bsf:a aac_adtstoasc <dest>.mp4` to pull down and remux the stream. Without ffmpeg a clear error is returned rather than a silent failure.
