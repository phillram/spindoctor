...
**Detecting staleness instead of just reporting "registered."** Since the `.bat`'s content can still go stale on upgrade (even though its file location no longer does), `introvideo.autorun_status()` returns an `AutorunStatus(registered, stale)` rather than a bare bool. `stale` is only ever computed on a frozen install (a source install's `.bat` calls bare `spindoctor` — nothing version-specific to go stale): the `.bat` is missing outright, or its content doesn't contain the exact string `_sibling_spindoctor_exe()` currently resolves to (a plain substring check against the `.bat`'s text — no parsing). `gui.py` mirrors the identical check for the wheel-refresh feature via `_refresh_exe_refs()` (factored out specifically so `_write_refresh_bat` and the staleness check can never drift against each other) checking all three of fav/recent/stats are present in the `.bat`. Both GUI status labels surface a stale registration explicitly, in an amber `Warn.TLabel` ttk style, with the exact fix ("click Enable auto-run again" / "click Schedule auto-refresh again") rather than a misleading plain "enabled" — requested directly by the user immediately after confirming the path-stability fix above, reasoning that a silently-broken-again registration after a future upgrade would be just as hard to diagnose as the original bug.
