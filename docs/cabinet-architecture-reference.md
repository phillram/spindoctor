...
**No ordering guarantee between the logon task and HyperSpin's own launch — confirmed on a real cabinet to matter, mitigated two ways, not eliminated.** The logon task and however HyperSpin itself currently auto-launches (this cabinet: the exe dropped directly in `shell:startup`) both fire around login with no strict OS-level ordering guarantee between the two. Initially disclosed as a theoretical low-risk tradeoff; real-world testing showed it wasn't theoretical at all — the swap failed on **every single reboot**, because HyperSpin was still holding `intro_video_target` open playing the *previous* intro video at the exact moment the logon task ran, and the resulting sharing-violation `OSError` was unhandled. Two complementary mitigations, both landed together: `swap_video` retries the copy (10 attempts, 1s apart) rather than failing on the first collision, and `install-autorun --delay-minutes N` (`schtasks /DELAY`, exposed in the GUI as **Delay after login (minutes)**, default `1`) pushes the whole task later so it starts *after* HyperSpin's intro clip has likely already finished — a bigger, coarser safety margin than the retry alone. SpinDoctor still intentionally does **not** eliminate the race at its source by scanning/moving the user's existing Startup-folder entry or chain-launching HyperSpin itself from the swap task: guessing wrong about an unfamiliar Startup-folder shortcut, or replacing however HyperSpin currently launches, risks breaking whether the cabinet boots at all — a far worse failure mode than a delayed or briefly-retried file copy. `introvideo swap --apply` (CLI or the GUI's **Swap now** button) lets you verify the mechanism works by hand at any time without waiting for a reboot.
