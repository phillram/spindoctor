...
```
ffmpeg -protocol_whitelist file,http,https,tcp,tls,crypto -i <url> -c copy -movflags +faststart <dest>.mp4
```

`-protocol_whitelist` enables HTTPS segment fetching from Steam's Akamai CDN. `-c copy` stream-copies both video and audio tracks without re-encoding — this is critical for CMAF/fMP4 segments (which Steam's Akamai CDN now uses): re-encoding audio with `-c:a aac` introduces timestamp discontinuities between the copied video track and the re-encoded audio, causing ffmpeg to silently abort the mux after 2–3 seconds while still exiting 0. The symptom is a file that appears to write successfully but plays only a few seconds. `-movflags +faststart` places the moov atom at the start for Windows Media Player seek compatibility. Without ffmpeg a clear error is returned rather than a silent failure.

**HLS quality variants and size:** Steam's HLS master playlist (`hls_h264` URL ending in `_master.m3u8`) contains `EXT-X-STREAM-INF` entries for multiple resolutions. `_pick_hls_variant(master_url, max_height, session)` parses these entries, selects the highest-resolution variant that fits within `max_height`, and returns its URL. If parsing fails, it falls back to the master URL (which ffmpeg resolves to the best variant automatically). The CLI flag `--hls-quality best|1080p|720p|480p|360p` (default `best`) controls this. At 1080p a long trailer (e.g. Submachine Legacy at 11:13) can reach 400+ MB; 480p brings the same trailer to ~25 MB. The GUI exposes this as a Quality dropdown (`Best (1080p)` / `720p` / `480p` / `360p`) next to the Overwrite checkbox in the Steam apply row.
