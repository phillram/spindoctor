...
**Curate** — thin out region/revision duplicates, prune library caches, and manage ignore lists. Three sections: **Curate region/revision variants** wraps `spindoctor curate` (with regions, prefer-revision latest/oldest, --include-proto, archive vs delete, dry-run by default) plus Undo, List manifests, and a **Preview (interactive)…** button that opens a Toplevel with a `☑/☐` per-row keep/skip toggle so you can veto specific retirements before committing. **Cache cleanup** wraps `cleanup categories / audit / run` with an optional `--older-than DAYS` field. **Ignore list** wires up `ignore add / remove / list` with system + game-name fields, plus a **View / un-ignore…** button that opens a click-to-un-ignore viewer with a system dropdown and a multi-select listbox.
