...
### Generating a full pattern batch (`scripts/generate_lwax_patterns.py`)

`ledblinky lwax fade` above builds one uniform fade. For a whole **library** of ready-made animations — directional sweeps, radial pulses, rain, cyclones, heartbeats, strobes, ripples, plasma, twinkles, a solid-colour "breathe" library, and more — run the batch generator. **This is a plain Python script, not an AI feature: anyone can run it, and it produces identical output every time.**

```bat
:: from a checkout of this repo (needs Python; the spindoctor package importable)
