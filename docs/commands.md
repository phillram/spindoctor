...
**Important:** `generate` now writes `Colors.ini` entries in LedBlinky's native named format (`P1_BUTTON1=Red`, `P1_JOYSTICK=White`) by looking up each default color's name from `Color-RGB.ini`. Older versions of SpinDoctor wrote a legacy hex format (`ledcolor1=FF0000`) that LedBlinky itself cannot read, causing all games to display default/white colors. If you have a `Colors.ini` generated by an older version, run `spindoctor ledblinky colors normalize --apply` once to convert it.
...
|---------|-------------|-----|
| All buttons white, coin dark | LedBlinky using DEFAULT control group | Check XML for per-ROM entry; verify RL sends correct ROM name |
...
| No colors.ini entry found | ROM never ran through `generate` or `fill-defaults` | Run generate + normalize + fill-defaults |
