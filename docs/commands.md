...
**`--delay-minutes N`** maps to `schtasks /DELAY` — **worth setting** (e.g. `--delay-minutes 1`), not just a rarely-needed knob: see the ordering caveat below. Confirmed on a real cabinet: without a delay, the swap can consistently lose a race with HyperSpin's own intro-video playback, since both fire around the same login moment on every single boot, not just occasionally.

**Ordering caveat:** the logon task and however HyperSpin itself currently auto-launches (e.g. a shortcut in `shell:startup`) both fire around login with no strict ordering guarantee between the two OS mechanisms. `swap_video` retries the copy for a few seconds if `intro_video_target` is locked (HyperSpin can still be playing the *previous* intro at that exact moment), but a `--delay-minutes` head start avoids the collision in the first place, more reliably, since it gives HyperSpin's whole intro clip time to finish rather than just a few seconds. SpinDoctor deliberately does not touch your existing Startup-folder entry or chain-launch HyperSpin itself to avoid the much larger risk of SpinDoctor guessing wrong about something that gates whether the cabinet boots at all.
