...
- **`spindoctor ledblinky admin-leds`** — new command group to manage the admin buttons that stay lit *during gameplay* — the always-active MAME UI controls in `LEDBlinkyControls.xml` (**Exit** = `UI_CANCEL`, **Pause** = `UI_PAUSE`, **Select** = `UI_SELECT`). Subcommands: `show` (default), `set` (uniform colours across all games — `--exit`/`--pause`/`--select` take a palette colour or `off`), `randomize` (a random colour per control group, `--seed` reproducible), and `add`/`remove` (insert or strip the admin controls for a console/emulator, e.g. to light admin buttons on a system that has none). All are dry-run by default, auto-backup, palette-validated, and accept `--emulator NAME` to scope to one console. This is the correct lever for in-game admin lighting on cabinets whose admin buttons are UI controls rather than `Colors.ini` `P{n}_BUTTON` keys — e.g. `admin-leds set --select off --apply` gives the "only usable buttons lit" arcade default (Exit + Pause stay lit). GUI: an **In-game admin LEDs** block in the LEDBlinky tab's Step 6 (uniform mode). Covered by `tests/test_ledblinky_admin_leds.py`.
