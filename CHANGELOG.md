...
- **"(Not in wheel)" badge in all system dropdowns.** System pickers across every GUI tab now annotate any system that exists as a folder in `roms_dir` or `databases_dir` but is absent from `Main Menu.xml` with a `(Not in wheel)` suffix. This makes stale or consolidated system remnants immediately visible so users can tell which system name is actually active on the wheel. The suffix is stripped before any CLI command runs — file paths and command arguments are never affected.
