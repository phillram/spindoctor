...
- **GUI window icon.** A simple two-tone vinyl-disc mark with a bold `S` on the label, generated by `scripts/make_icon.py` and shipped under `spindoctor/assets/`. Loaded via `iconbitmap` on Windows (multi-resolution `.ico`) and `iconphoto` elsewhere; missing-asset path is non-fatal.
