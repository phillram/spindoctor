...
- **`spindoctor.lwax.build_wave()` and `build_rain()`** — two more generic animation primitives alongside `build_color_cycle()`, used to generate this cabinet's directional sweep, radial pulse, and rainfall `.lwax` animations. `build_wave()` moves a leading color (with an optional trailing second color) across an ordered list of control-label groups, wrapping continuously — the same function covers left/right sweeps, up/down sweeps, and radial pulses; direction is just the order the groups are passed in (reverse the list to reverse direction). `build_rain()` runs independent, randomly-timed drop sequences per group so multiple "columns" flash out of sync, deterministic given a seed. Not exposed as CLI subcommands (yet) — the column/row/ring groupings are cabinet-specific data, generated today via a one-off script rather than new CLI flags, per user preference to avoid growing the CLI surface for effects that are mostly about picking colors/timings once.
