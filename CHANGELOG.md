...
- **HyperSpin theme zips for synthetic wheels** — SpinDoctor now bundles and installs `Media\Main Menu\Themes\<SystemName>.zip` for `Favorites`, `Most Played`, and `Recently Played`. Without a theme zip HyperSpin silently skips the attract-mode audio/video entirely. Each zip contains only `Theme.xml` + `Info.txt` (matching the MAME "Cinematic" minimal-theme pattern). The `<video>` element uses `w="1" h="1" x="1" y="1"` (one invisible pixel in the top-left corner) so the MP4's audio track plays while the background PNG provides all the visual content — avoids a double-render artefact that occurred when the video overlay and background image showed the same frame simultaneously. Installed by `rebuild --apply` (skip-if-exists) and `mainmenu add --apply` (always overwrite). Theme zips ship via the `assets/*.zip` package-data glob.
