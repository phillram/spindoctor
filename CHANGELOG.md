...
- **GUI: Custom Wheels tab's "Auto-refresh on cabinet startup" section now shows its Windows Task Scheduler status without clicking anything.** Previously required clicking "Check task status" every time; now shows Status: enabled/disabled/not available immediately on the tab and refreshes automatically after Schedule/Remove/Check, matching the Intro Video tab's Auto-run status label.
- **Both auto-run status checks (Intro Video and Custom Wheels tabs) now detect a *stale* registration, not just enabled/disabled.** A task can be "registered" in Task Scheduler yet no longer actually work — e.g. after upgrading SpinDoctor into a new version folder without re-running `install-autorun`/re-clicking "Schedule auto-refresh," since the generated `.bat` still references the old install's executables. Both status labels now say so explicitly — "Outdated: Enable auto-run to fix" (Intro Video) / "Outdated: Schedule auto-refresh to fix" (Custom Wheels), shown in amber, each naming the exact button that fixes it — instead of a misleading plain "enabled." `introvideo.autorun_status()` now returns an `AutorunStatus(registered, stale)` instead of a bare bool; `introvideo uninstall-autorun` (dry-run) also surfaces staleness with a pointer to `install-autorun --apply` instead of removal.
