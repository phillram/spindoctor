...
- **New `#### Master control reference` table consolidating every control's physical label, LedBlinky label, Mini-PAC pin, key sent, PAC-LED64 board:port range, and RetroArch action into one row each (35 rows: the 27 LED-equipped controls plus the 8 joystick-direction keys).** Previously this data was split across four separate places (physical position table, pin-to-key table, button function reference tables, PAC-LED64 channel map) — exactly the kind of split that let the Mouse Right/Middle typo and the missing-bottom-row bug both go unnoticed until confirmed on real hardware. Explicitly does not include a "color" column since that's assigned dynamically per ROM via `Colors.ini` (or via the FE Controls Editor for the idle/frontend context), not a fixed property like the rest of the row.
