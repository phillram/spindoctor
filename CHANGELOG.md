...
- **`spindoctor ledblinky admin-leds`** — new command to show/set the admin buttons that stay lit *during gameplay* — the always-active MAME UI controls in `LEDBlinkyControls.xml` (**Exit** = `UI_CANCEL`, **Pause** = `UI_PAUSE`, **Select** = `UI_SELECT`). Run with no options to print the current state; `--exit`/`--pause`/`--select` take a palette colour or `off` and sweep every control group at once (dry-run by default, auto-backup). This is the correct lever for in-game admin lighting on cabinets whose admin buttons are UI controls rather than `Colors.ini` `P{n}_BUTTON` keys — e.g. `admin-leds --select off --apply` gives the "only usable buttons lit" arcade default (Exit + Pause stay lit). GUI: an **In-game admin LEDs** block in the LEDBlinky tab's Step 6. Covered by `tests/test_ledblinky_admin_leds.py`.
