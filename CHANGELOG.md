...
- **Daphne LaserDisc file layout documented in cabinet architecture reference.** Covers the three-file-type split (`.txt` framefile in `J:\Games\Daphne\`, chip ROMs in emulator `roms\` subfolder, VLDP video in `J:\Games\Daphne\vldp\<game>\`), framefile format (first line = relative VLDP path), common path mistakes (leading `/` makes path absolute, `..` prefix navigates to wrong parent), and a PowerShell bulk-repair command. ROM storage layout in the directory tree updated to show the Daphne folder structure.
