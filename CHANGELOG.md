...
- **`ledblinky admin-leds set` can now blank the Search and mouse admin buttons in-game** — `--search-off`, `--lmouse-off`, `--rmouse-off` (GUI: an optional "turn off" checkbox row in the LEDBlinky tab's in-game admin section). Unlike Exit/Pause/Select, these buttons aren't their own LED control — their key rides on another always-active control's `inputCodes` (e.g. Search's "/" is on `UI_PAUSE`, so the Search button lights whenever Pause does). "Off" removes just that button's keycode token, so Search goes dark while Pause stays lit. A button whose key isn't on any lit control is reported as already dark. Off-only and reversible from the `.bak`. Covered by `tests/test_ledblinky_admin_leds_offkeys.py`.
