...
- **Documented the RocketLauncher-only fix for `Escape` erroneously exiting PC games (`docs/cabinet-architecture-reference.md`).** The Exit button sends `Escape` at every layer, including RocketLauncher's `exitEmulatorKey := "Esc"`, so one `Escape` in a PCLauncher game both opens its menu and closes the game. Recommends a per-system passthrough chord — `~s & ~Esc` (P1 Coin + Exit, reusing the cabinet's existing swop-key idiom) — for the `PC Games` / DOSBox systems, matching the cabinet's existing `~LShift & ~Z` exit-combo style, so no panel key is consumed and console/arcade systems stay on `Esc`. Records the concrete edit confirmed against the cabinet's INIs: `Exit_Emulator_Key` under `[Exit]` in the **per-system** `Settings\<System>\RocketLauncher.ini` (note: that filename, not `<System>.ini`), leaving the global `Settings\Global RocketLauncher.ini` `Exit_Emulator_Key=Esc`. The Xpadder purge step likewise names the concrete setting — `Keymapper_Enabled=false` under `[Keymapper]` in `Settings\Global RocketLauncher.ini` — and flags that JoyIDs controller-ordering shares that block. Cross-linked from `docs/windows-10-migration.md`.
...
- **`doctor` can no longer be crashed by any single check.** Even after the `check_databases` hardening, other checks could still throw an unhandled exception and abort the whole run — e.g. `check_global_emulators --fix` propagating an `OSError` from `mkdir` when `rocketlauncher_dir` sits on an ejected/read-only drive. Every check is now run through a wrapper that turns an unexpected exception into a single FAIL row ("check crashed: …") so the rest of the report still renders. A diagnostic tool must never die on the thing it's diagnosing. Covered by `tests/test_health.py`.

- **`doctor` no longer crashes on a locked or mis-encoded database.** `check_databases` only caught `ValueError`, but on a live cabinet `load_database` can raise `PermissionError` (HyperSpin has the XML open), `OSError`, or `UnicodeDecodeError` — any of which took down the entire `doctor` run instead of showing one `✗` row. It now catches those and reports the offending system as FAIL while continuing. It also flags a database that parses but has **0 games** as WARN (previously a green "0 entries") — usually a truncated/empty XML written mid-crash.
- **`doctor`'s emulator check no longer reports OK when the emulator binary is missing.** The wheel-wiring check confirmed a system's `Default_Emulator` was *registered* in `Global Emulators.ini` but never that its `Emu_Path` exe actually exists — so a wheel whose emulator was uninstalled or left on a since-changed drive letter read as healthy while its games silently failed to launch. It now resolves `Emu_Path` (absolute, or relative to the RocketLauncher dir) and WARNs when the binary is absent. Conservative by design: paths it can't resolve on the current OS (an absolute Windows path checked from a dev machine) are left as OK rather than false-flagged.
- **`doctor`'s match-cache check is now case-insensitive and guards an unset `roms_dir`.** ROM staleness was compared case-sensitively, so a tagger that re-cased `Mario.zip`→`mario.zip` made `doctor --apply` prune a still-valid cache entry on the (case-insensitive) NTFS cabinet. And with `roms_dir` unset every entry looked stale, so `--apply` would have wiped the whole cache — it now skips with an INFO instead. Also added a fix hint to the `Global Emulators.ini` `--fix`-failure path.

- **`get_rom_extensions` no longer mis-resolves short system-folder names.** The fuzzy fallback matched a folder name against any extension key that merely *contained* those letters, longest-key-first — so a folder named `ST` (Atari ST) resolved to SNES's extensions (`.sfc/.smc`) because `"st"` appears inside `"supernintendoentertainmentsystem"` ("sy**st**em"), and `PS` matched `philipscdivideo`. This silently changed which files `audit`/`organize`/`curate` treat as valid ROMs for that system. Matching is now two-phase: prefer the longest known key *contained in* the folder name (so `Sega Genesis` → `genesis`, never `nes`), then fall back to the *shortest* key that contains the folder name (so `ST` → `atarist`).
- **`get_systems` no longer scans the current working directory when unconfigured.** `Path("")` resolves to `.`, so a `Config` with an empty `roms_dir`/`hyperspin_dir` scanned whatever folders happened to sit in the process's CWD (e.g. a repo checkout) and returned them as "systems". It now skips a scan whose configured dir is empty.
- **Synthetic wheels (Favorites / Recently Played / Most Played / stats wheels) now mirror media for games whose title contains a `:` or other Windows-forbidden character, instead of silently showing blank art.** Media files are written with a sanitized filename (e.g. `Metal Gear Solid VR Missions.png`), but the mirror compared against the raw game name (`Metal Gear Solid: VR Missions`), so it matched nothing and copied zero media for such games. The mirror now matches and writes by the sanitized stem — consistent with how HyperSpin itself resolves media.
- **Rebuilding a synthetic wheel no longer crashes when a favorited game's name contains a `:` (or `?`, `*`, …).** The per-game PCLauncher INI was written as `<raw name>.ini`, an invalid Windows filename that raised `OSError` mid-rebuild (common for PC/Steam titles like `Submachine: Legacy`). The INI filename is now sanitized (the launch parameters still carry the raw source name), and the orphan-cleanup allowlists were aligned so they never delete the freshly written sanitized files.
- **"fade" images are now mirrored into synthetic wheels.** `Images/Artwork4` (where `fade` media lives) was missing from the mirror list, so fade art never appeared on synthetic wheels.
- **The synthetic-wheel per-game theme fallback now finds a lowercase `default.zip`.** The fallback looked only for `Default.zip` (capital D), but SpinDoctor's own `fill_default_theme` and the HyperSpin/RocketLauncher convention use lowercase `default.zip` — so on case-sensitive storage the fallback silently did nothing. Both casings are now accepted.
- **`audit` now matches ROMs to database entries case-insensitively.** The cabinet's NTFS filesystem and HyperSpin both treat name case as insignificant, but `audit_system` compared ROM stems to DB entry names case-sensitively — so a ROM `Pac-Man.zip` with a DB entry `<game name="pac-man">` (common after hand-edits or legacy-list imports) was reported as **two** problems at once ("ROM not in database" *and* "database entry has no ROM"), and could prompt a needless metadata re-fetch / duplicate entry. Matching now folds on lower-case (DB spelling kept as canonical), so a correctly-matched game counts once.
- **`audit`'s fuzzy-match results are now deterministic.** Entries were iterated in `set` order (randomized per process via hash seed), so on ambiguous inputs the greedy fuzzy-match pass could claim a different DB entry on each run of the same unchanged data. Entries are now processed in sorted order.
- **Recursive PC-game scanning no longer silently drops a game on a title collision.** Under `title_strategy: "stem"`, two different game folders that each ship a same-named launcher (e.g. both `Launcher.exe`) derived the same title, and the second was discarded with no diagnostic — the game just never appeared in the audit. It now emits a warning naming the kept and skipped paths so you can rename a folder or change the strategy. Intended deduplication in the default `"smart"` mode (a launcher plus its desktop shortcut) stays silent.
### Added
...
- **`doctor` now reports orphan media.** A new **Orphan media** check counts leftover media files (wheels, videos, themes) with no matching game — disk bloat left behind after games were removed. INFO-only and never deletes anything (doctor's "never deletes media" contract); it points at `find-orphan-media --all` for the preview-and-remove flow. Covered by `tests/test_health.py`.

- **`doctor` now checks the intro-video randomizer.** A new **Intro video** check (only when the feature is configured) catches failures that otherwise have no symptom until someone notices the boot video stopped changing: an empty pool (`introvideo swap` is a silent no-op by design → WARN), a `intro_video_target` whose folder no longer exists (WARN), and a logon auto-run task that's registered but stale after a portable upgrade (WARN → re-run `introvideo install-autorun --apply`). The auto-run probe is Windows-only and skipped cleanly elsewhere. Covered by `tests/test_health.py`.

- **`doctor` now reports LEDBlinky control coverage for the MAME set.** A new **LED coverage** check shows how many MAME games already have LEDBlinky control/color entries and how many *could* be generated (`ledblinky generate` → WARN when there are any). It is deliberately **cache-only** — it never triggers a fresh `mame -listxml` (which can take minutes) from `doctor`; without cached control data it just points at `ledblinky audit`. Gated on `ledblinky_dir` + `mame_executable`. Covered by `tests/test_health.py`.

- **`doctor`'s LEDBlinky check is deeper, and wheel-wiring now finds reverse orphans.** The LEDBlinky check previously only confirmed two files parsed; it now also flags a missing `Color-RGB.ini` (without it `ledblinky generate` silently degrades to a legacy color format LedBlinky can't read) and runs the Search/Genre/Favorites crash scan (a LedBlinky process hook in a menu's `Settings.ini`, or a missing `LEDBlinkyControls.xml` entry — the documented cause of HyperSpin crashing on those menus — with `ledblinky fix --apply` as the remedy). Separately, the wheel-wiring check now reports **reverse orphans**: systems fully set up under `Databases\` but missing from the Main Menu, so invisible in HyperSpin. Covered by `tests/test_health.py`.

- **`doctor` now checks PC-game launchability.** The wheel-wiring check treats `Default_Emulator=PCLauncher` as automatically OK and never looks deeper — so a PC system whose per-game launch INIs are missing or point at a moved executable (the "Cannot find this Application" / stale drive-letter bug) read as healthy. A new **PC games** check walks the per-game `Modules\PCLauncher\<System>\<game>.ini` files and flags games with no INI (FAIL → `add-pc-system`) or whose `Application=` path no longer exists on disk (FAIL → `pc-rename`). Existence checks are conservative — an absolute Windows path evaluated on a non-Windows machine is not flagged. Covered by `tests/test_health.py`.

- **`doctor` now checks lightgun wiring.** For every system marked `lightgun: true`, a new **Lightguns** check verifies the RocketLauncher DemulShooter wiring: a system flagged for a light gun but with no `Pre_Launch_App` (the game launches with no aiming device) is a FAIL, and a missing `Post_Launch_App` teardown (DemulShooter left running, which can break the next launch) is a WARN. A configured-but-missing `demulshooter_path` is also flagged. Diagnosis-only — each finding names the exact `lightgun configure` command. Covered by `tests/test_health.py`.

- **`doctor` now checks per-wheel wiring, not just install-wide state.** The previous checks (databases, `Global Emulators.ini`, media skeleton) could all pass while an individual wheel was still broken — most visibly the "Cannot find `<System>.ini`" failure, which `doctor` never looked for. A new **Wheel wiring** section walks `Main Menu.xml` and, for every wheel, verifies the HyperSpin `Settings\<System>.ini` (required to *open* the wheel), the console-level `Media\<System>\Themes\default.zip` fallback theme, the RocketLauncher emulator mapping (`Default_Emulator` resolving to a known executable — games won't launch otherwise), and that the wheel has a database. `doctor --apply` now also writes any missing `Settings\<System>.ini` and `default.zip` (both idempotent, existing files untouched); the emulator-mapping gap is diagnosis-only, printing the exact `config system set` + `generate-config` fix since SpinDoctor can't install an emulator for you. Built-in Main Menu entries like `Search` (`exe="true"`) are excluded. Covered by `tests/test_health.py`.
...
- **`ledblinky admin-leds set` can now toggle the Search and mouse admin buttons in-game** — `--search-off/-on`, `--lmouse-off/-on`, `--rmouse-off/-on` (GUI: an optional Search / Left Mouse / Right Mouse dropdown row — `off (dark)` / `on (lit)`). Unlike Exit/Pause/Select, these buttons aren't their own LED control — their key rides on another always-active control's `inputCodes` (e.g. Search's "/" is on `UI_PAUSE`, so the Search button lights whenever Pause does). `off` removes just that button's keycode token (Search goes dark, Pause stays lit); `on` re-attaches it to the Pause control. `on` is scope-sensitive — with no `--emulator` it lights the button on every console. A button whose key isn't on any lit control is reported as already dark (off) or no-host (on). Reversible from the `.bak`. The Search↔Pause coupling was diagnosed as a GameCube + Wii config quirk (only those two consoles bundle Search's "/" onto UI_PAUSE). Covered by `tests/test_ledblinky_admin_leds_offkeys.py`.
...
- **GUI: Custom Wheels tab's "Auto-refresh on cabinet startup" section now shows its Windows Task Scheduler status without clicking anything.** Previously required clicking "Check task status" every time; now shows Status: enabled/disabled/not available immediately on the tab and refreshes automatically after Enable/Disable/Check, matching the Intro Video tab's Auto-run status label. The two tabs' buttons are also now identically named — **Enable auto-run** / **Disable auto-run** — since they wrap the same Task Scheduler mechanism (previously the Custom Wheels tab used "Schedule auto-refresh" / "Remove scheduled task").
- **Both auto-run status checks (Intro Video and Custom Wheels tabs) now detect a *stale* registration, not just enabled/disabled.** A task can be "registered" in Task Scheduler yet no longer actually work — e.g. after upgrading SpinDoctor into a new version folder without re-clicking **Enable auto-run**, since the generated `.bat` still references the old install's executables. Both status labels now say so explicitly and identically — "Outdated: Enable auto-run to fix" (shown in amber) — instead of a misleading plain "enabled." `introvideo.autorun_status()` now returns an `AutorunStatus(registered, stale)` instead of a bare bool; `introvideo uninstall-autorun` (dry-run) also surfaces staleness with a pointer to `install-autorun --apply` instead of removal.
- **`spindoctor ledblinky lwax batch`** — new CLI command that generates the whole LED pattern library (dry-run by default, `--apply` to write, `--output` to pick the folder). Its logic lives in a new shared module `spindoctor/lwax_patterns.py`; `scripts/generate_lwax_patterns.py` is now a thin shim over it. Covered by `tests/test_lwax_patterns.py`.
- **GUI: new "Step 10 — LED Animations (.lwax)" section on the LEDBlinky tab** — a **Generate pattern library** button (`ledblinky lwax batch`) and a single-fade field + **Generate fade** button (`ledblinky lwax fade`), with the unsigned-file / Save-As reminder inline. Previously these were reachable only via the Custom Command dropdown.
- The `ledblinky --help` command list now includes the previously-omitted `setup`, `inspect-rom`, and `lwax` subcommands.
- **`spindoctor ledblinky lwax calibrate`** — new subcommand that lights each control a distinct, named colour (admin row by default: red/green/blue/yellow/magenta/cyan) and prints a label→colour legend, as a button-mapping aid. Sign it, assign as the FE/screen-saver animation with `LightFEControls=0`, and compare the panel to the legend to confirm which physical button carries which label.
- **`spindoctor ledblinky admin-leds`** — new command group to manage the admin buttons that stay lit *during gameplay* — the always-active MAME UI controls in `LEDBlinkyControls.xml` (**Exit** = `UI_CANCEL`, **Pause** = `UI_PAUSE`, **Select** = `UI_SELECT`). Subcommands: `show` (default), `set` (uniform colours across all games — `--exit`/`--pause`/`--select` take a palette colour or `off`), `randomize` (a random colour per control group, `--seed` reproducible), and `add`/`remove` (insert or strip the admin controls for a console/emulator, e.g. to light admin buttons on a system that has none). All are dry-run by default, auto-backup, palette-validated, and accept `--emulator NAME` to scope to one console. `randomize` also takes `--games ROM1,ROM2 --emulator NAME` to force true per-game variety (clones the emulator's DEFAULT group for each named ROM so it draws its own colors). This is the correct lever for in-game admin lighting on cabinets whose admin buttons are UI controls rather than `Colors.ini` `P{n}_BUTTON` keys — e.g. `admin-leds set --select off --apply` gives the "only usable buttons lit" arcade default (Exit + Pause stay lit). GUI: an **In-game admin LEDs** block in the LEDBlinky tab's Step 6 with a per-console scope dropdown. Covered by `tests/test_ledblinky_admin_leds.py`.
- **`docs/reference/LEDBlinkyControls.sample.xml`** — committed trimmed sample of this cabinet's LEDBlinky control database, showing the per-group `UI_*` admin block and the global control-definition block, as a reference and test fixture for `admin-leds`.
- **`scripts/generate_lwax_patterns.py`** — one-off generator that emits a colour-diverse batch of LEDBlinky `.lwax` animations into `~/Downloads/spindoctor-lwax-patterns/`. 22 effect families × 5 variants (fade, sweep, rain/confetti, scroll, fill, drain, checker, radial pulse, cyclone, race-from-centre, heartbeat, strobe, marquee, VU-meter bounce, row-blink, ripple, spiral, comet, lava-lamp/plasma, twinkle, candle, breathing-gradient), each with one moving/fading rainbow variant and a slow/medium/fast spread; plus a `breathe_*` solid-colour library (standard, `vivid_*`, `pastel_*`, `breathe_rainbow`) and two colour-cycle files (`breathe_cycle`, `pulse_cycle`) that step through several unique solid colours and loop. Every fixed-colour variant uses a globally unique, hue-strided colour so families don't clump. Geometry-aware effects (ripple/spiral) use an approximate 2D position map derived from the layout groups. Builds on the `spindoctor.lwax` builders plus local pulse/plasma/noise helpers; files are raw/unsigned and still need the Animation Editor Save-As signing step on the cabinet.
- **`docs/reference/LEDBlinkyInputMap.xml`** — committed reference copy of this cabinet's corrected LED input map (matching the [Master control reference](docs/cabinet-architecture-reference.md#master-control-reference)), so `.lwax` generators can run without re-exporting it from the cabinet.

### Fixed

- **The logon-task `.vbs` launcher never actually ran on a real Windows login — the actual root cause of the intro video never swapping automatically, at any delay. Confirmed fixed on the reporting cabinet: three different videos across three separate reboots after applying this fix, versus zero changes across many reboots before it.** A backslash-escaping mistake in the Python code generating the `.vbs` shim made its runtime `InStrRev` path-search always fail to match, silently collapsing the computed `.bat` path down to a bare filename with no folder. A bare relative filename only resolves if the caller's working directory happens to be the right folder — true when double-clicking the `.vbs` in Explorer (which is why manual testing kept "working"), false for a Task Scheduler action (which is why every logon-triggered run silently failed to even find the `.bat`, regardless of delay or retry settings, with nothing in Task Scheduler's history and no error anywhere). Fixed by having the `.vbs` embed the full, already-known `.bat` path directly at generation time instead of re-deriving it at runtime — removes the dependency on working directory, and the whole class of bug, entirely.
- **`introvideo swap` now retries if `intro_video_target` is locked, instead of silently failing every time — and the retry window is sized off real intro-clip lengths, not a guess.** Confirmed and reproduced on demand on a real cabinet: HyperSpin holds `intro_video_target` open for a clip's **entire playback** — running `introvideo swap --apply` while a clip is actively playing fails immediately with `PermissionError: [WinError 32] The process cannot access the file because it is being used by another process`; running it again once the clip finishes succeeds instantly. Since the logon task fires around the same login moment HyperSpin does, it was consistently losing this race on every single reboot. Intro clips on the reporting cabinet ranged ~10 seconds to ~2 minutes, so `swap_video` now retries for `SWAP_RETRY_ATTEMPTS` × `SWAP_RETRY_DELAY_SECONDS` = 90 × 2s = **3 minutes** (previously 10 × 1s = 10s, which undershot anything but the shortest clips) before giving up (`IntroVideoError` if every attempt fails).
...
- **The intro-swap logon task no longer breaks on every SpinDoctor upgrade.** The generated `.bat`/`.vbs` used to live next to the running `spindoctor.exe` — but portable Windows installs unzip each release into its own version-numbered folder (e.g. `spindoctor-win10-v2.11.0\`), so the registered Task Scheduler entry silently pointed at a folder that could vanish (or just become the *wrong* version) on the next upgrade. Both files now always live in `~/.spindoctor/` — the same stable location `config.json` already uses — so the Task Scheduler registration itself never needs to change across upgrades again. The file *contents* (which reference the per-version `spindoctor.exe`) still need a one-time refresh after upgrading — re-run `introvideo install-autorun --apply` (or click **Enable auto-run** again in the GUI) once per upgrade.
- **The GUI's older wheel-refresh auto-run feature (Custom Wheels tab) had the identical `.vbs` path-derivation bug as the intro-swap task above, plus the same fickle next-to-the-exe storage location** — found by re-auditing this feature for the same class of bug once the intro-swap root cause was confirmed. Same fixes applied: the `.vbs` now embeds the `.bat`'s full path directly instead of re-deriving it at runtime (the backslash-escaping mistake was byte-for-byte identical), the `.bat`/`.vbs` pair now always lives in `~/.spindoctor/`, and the `.bat` now tracks a failure flag across all three refresh steps (Favorites/Recently Played/Most Played) and exits with it, so Task Scheduler's Last Result is meaningful. **Existing installs should re-click "Enable auto-run"** to regenerate the launcher files with these fixes (this button was renamed from "Schedule auto-refresh" in the same PR — see above).
...
- **GUI Console (Custom Command tab) silently corrupted any typed command with quoted arguments on Windows.** `_run_custom` splits the typed command with `shlex.split(posix=False)` on Windows (deliberately, so backslashes in Windows paths and spaces-in-quotes survive) — but that mode does not strip the quote characters from a quoted token, only keeps it as one piece. A command like `emulator-title set "Daphne" "SDL_app"` was passed through to the CLI with the quote characters left inside each argument (`'"Daphne"'` instead of `'Daphne'`), so `emulator-title set` stored a corrupted key (`"Daphne"`, quotes and all) that never matched anything at lookup time — a completely silent failure (`emulator-title list` even echoed the quotes back, but nothing downstream ever matched against them). This is what made the Daphne `FadeTitle` workaround above appear not to work even after being set correctly via the GUI. Added `_dequote_windows_shlex_tokens()`, which strips exactly one matching pair of surrounding quotes per token after the split, leaving interior characters (including backslashes) untouched.
- **Setup tab (and first-run wizard): Browse buttons could open a folder that had nothing to do with the field's current value.** `askdirectory`/`askopenfilename` were handed the field's raw text as `initialdir` even when that path didn't exist on disk (an unfilled field's hardcoded Windows default, a typo, a drive that isn't mounted) — Tk then silently falls back to whatever directory the dialog last happened to be in, which reads as "Browse opens a random folder." `_browse_dir` now resolves the nearest *existing* ancestor of the field's current value (falling back to the home directory only if nothing in the path exists), so Browse reliably lands on the configured folder — or its closest still-present parent — instead of somewhere unrelated.
- **Removed every `…` from the GUI** — button labels and menu items (`Browse…`, `Restore DB backup…`, `Pick subset…`, `Run first-run wizard…`, `File → View logs & manifests…`, `File → Browse HyperSpin themes…`, `Help → First-run setup…`, etc. — 33 buttons plus 3 menu entries), status-bar progress text (`Scanning library…`, `Running doctor…`, `Contacting ScreenScraper and TheGamesDB…`, `Step N/M: <name>…`, `Stopping…`, etc.), a file-dialog title (`Save audit CSV as…`), a keyboard-shortcut range (`Ctrl+1 … Ctrl+9`, now an en dash), a form hint's example list (`RetroArch, Daphne, …` → `..., etc.`), and the "more than 10 items" truncation marker in two confirmation dialogs (now spelled out as `(+N more)` instead of a bare `…`). Also removed the ellipsis the status bar appended when hard-truncating an overflowing command string to fit the available width — it now just cuts the text off at the fitting prefix with no trailing mark. v2.10.2 already dropped the ellipsis from the Intro Video tab's "Add video(s)…" for the same reason (didn't match the rest of the GUI) but never swept the rest of the app.
...
- **`cabinet-architecture-reference.md`** — new *HyperSpin Startup/Exit Orchestration* section documenting the community "HyperSpin Startup Script" INI (the `[Startup]`/`[Exit]` mechanism many cabinets use to launch/kill DS4Windows, Xpadder, antimicro, CabVol, HyperSearch around HyperSpin) and the typical layered input stack (controller → mapper → keystrokes vs. arcade panel → keystrokes), cross-linked with the existing DS4Windows and RetroArch input sections. Also adds a new *HyperSpin frontend menu controls* table (Player 1/Player 2 Up/Down/Skip Up/Skip Down/Skip Up-Down by number/HyperSpin/Genre/Favorites/Start/Exit), confirmed against the live cabinet — including that neither player's "Start" action is bound to that player's own physical Start button (P1's fires on the admin Select key, P2's fires on P1's physical Start button), which is the real config, not a doc error.
- **New guide: PS4 controller dual-mode input via DS4Windows Auto Profiles** (`docs/controller-input.md`) — one DS4Windows setup that sends keystrokes for HyperSpin menu navigation and switches to XInput for in-game play, auto-switched on window focus. Documents why native HyperSpin joystick nav and a separate Xpadder/antimicro mapper were both dropped, the Player 1/2 keyboard profile, the "run at Windows login, independent of HyperSpin" requirement, and the resulting simplified Startup Script `[Startup]`/`[Exit]` INI. Linked from `docs/index.md` and `docs/windows-10-migration.md`; corrects the now-stale "DS4Windows is started alongside HyperSpin and exits when HyperSpin exits" framing in `cabinet-architecture-reference.md` (kept there as the failure mode for cabinets still on that setup).
...
- **`introvideo shuffle`** — randomizes the playback order of the Intro Video Randomizer's registered videos in `Random.ini` (`FileList=`/`RandomList=`). Adds/removes/deletes nothing, only reorders — useful because the third-party randomizer script itself may not vary its pick from one boot to the next, so pre-shuffling the list is what actually changes playback order across sessions. `--seed` gives a reproducible order (mainly for tests); `--apply` commits, dry-run by default. GUI: new **Shuffle order** button on the Intro Video tab.
- **GUI: `<Random>` is now a real, selectable value in all three LedBlinky animation dropdowns** (FE active, screen saver, in-game unused buttons), and gets written to `Settings.ini` literally — LedBlinky's own "pick a different animation each time" feature. Previously the FE/screen-saver dropdowns conflated the literal string `"<Random>"` with a "leave this key unchanged" sentinel, so selecting it from the list (it also wasn't in the dropdown's actual value list) never wrote `<Random>` to `Settings.ini` — there was no way to explicitly turn LedBlinky's random-animation feature on through the GUI. The "leave unchanged" sentinel is now the distinct, explicit `(leave unchanged)` option.
...
- **`spindoctor.lwax.build_wave()`, `build_rain()`, `build_rainbow_scroll()`, `build_fill()`, `build_drain()`, `build_alternate()`, and `merge_animations()`** — generic animation primitives alongside `build_color_cycle()`, used to generate 67 `.lwax` animations for this cabinet: the original 16 requested effects (directional sweeps, rainfall, radial pulses, a ping-pong "breathing" pulse, confetti, a two-color race from a shared center, a scrolling rainbow, a Cylon-style scanner, clockwise/counter-clockwise "cyclone" racetrack loops, an accumulating combo-meter fill, and a heartbeat brightness pulse), 48 Fire/Ice/Toxic color-themed variants of those, and 4 more new effect types (a countdown/fuse, a "chase the leader" rivalry loop, a strobe alert, and a checkerboard blink). `build_wave()` moves a leading color (with an optional trailing second color) across an ordered list of control-label *groups*, wrapping continuously — the same function covers sweeps, radial pulses, ping-pong scanners (double the group list as `groups + groups[-2:0:-1]`), racetrack "cyclone" loops (concatenate two parallel rows, one reversed), and "chase the leader" (a large `lag` so two colors stay a fixed distance apart forever instead of one closely trailing the other); direction is just the order groups are passed in. A group can hold more than one label when two controls are lit at the same physical spot (e.g. a Start button sharing an action button's position) — `build_rainbow_scroll()` takes the same grouped-position convention (not a flat label list) so co-located labels always get the identical hue. `build_rain()` runs independent, randomly-timed drop sequences per group (one-label-per-group turns it into confetti instead of rain). `build_fill()` accumulates lit groups instead of moving a single position, holds, flashes, and loops; `build_drain()` is its inverse (starts full, extinguishes groups in order — a countdown/fuse effect). `build_alternate()` swaps two groups between two colors in lockstep with no motion at all (a checkerboard blink). `merge_animations()` layers independently-built, frame-synchronized animations into one (e.g. two waves racing from a shared center in different colors, using mismatched group-list lengths on each side by crossing their `frames_per_step` values so both land on the same total frame count). Not exposed as CLI subcommands — the column/row/ring/loop groupings are cabinet-specific data, generated via a one-off script rather than new CLI flags, per user preference to avoid growing the CLI surface for effects that are mostly about picking colors/timings once.
...
- **Corrected the `.lwax` signature section: the "missing signature" check only gates LedBlinky Config's own UI, not runtime playback — confirmed on this cabinet.** Assigning a raw, unsigned `.lwax` directly to `FELWAFile`/`FEScreenSaverLWAFile`/`GamePlayLWAFile` in `Settings.ini` and launching HyperSpin plays it correctly; the error only surfaces if LedBlinky Config itself tries to load/browse/select the file within its own animation-management UI. The Animation Editor Open → Save As round-trip documented in that section is therefore only needed to manage the file *through* LedBlinky Config, not a prerequisite for the animation to actually run.
- **Physical button layout table's cells now carry each control's key in parentheses again** (e.g. `P1 Start (`R`)`, `Select (`Enter`)`), after the consolidation below first stripped them out in favor of a pure position table. The key shown here is a convenience copy, not a second source of truth — the table's intro now says explicitly that the Master control reference wins if the two ever disagree, so this doesn't reopen the drift risk the consolidation closed.
- **Consolidated all per-control hardware data down to exactly two source-of-truth tables.** The cabinet's control data had grown across six overlapping tables (physical position, joystick keybinds, pin-to-key mapping, admin/P1/P2 button-function, and two per-board PAC-LED64 channel maps) — the exact kind of split that let the Mouse Right/Middle typo and the missing-bottom-row bug both go unnoticed until confirmed on real hardware. Now there are two, each owning a distinct set of facts with no overlap: a **Physical button layout** table (position only) and a **Master control reference** table (LedBlinky label, Mini-PAC pin, key sent, PAC-LED64 board:port range, and RetroArch action — one row per control, 35 rows). The four redundant tables were removed and their unique prose (the `inputCodes` upstream-error note, swop-key secondary functions, SNES button-naming note, key/letter inventory, RGB-triplet/unwired-port notes) folded in beside whichever survivor owns it. The master table deliberately omits an LED "color" column, since color is assigned dynamically per ROM via `Colors.ini` (or via the FE Controls Editor for the frontend context), not a fixed per-control property.
...
- **This cabinet's physical control panel layout documented, confirmed against an actual photo.** New subsection of "LEDBlinky Animation Files (.lwax)" recording the confirmed Start/Coin/trackball/admin-row positions (Start/Coin are not mirrored between players — P1's Start/Coin sit over its joystick/B1, P2's over B2/B3) and the meta-lesson: a real photo of the hardware resolved the layout in one pass where two rounds of ASCII-table-then-diagram correction hadn't. Ask for a photo before starting from a text description, when one exists.
- **`#### Physical button layout` (Mini-PAC/PAC-LED64 hardware section) replaced with a corrected position table.** The original hand-drawn ASCII diagram had Player 1's Coin/Start order backwards and assumed Player 2 mirrored Player 1's Coin/Start placement — both wrong per the panel photo above. Box-drawing art also isn't a reliable format for something this position-sensitive; replaced with a plain column/tier table covering every physical control including the joysticks (which the LED-only layout in the `.lwax` section skips). Letter-inventory footnote split into its own `#### Letter assignments` subsection so the layout table isn't sharing space with an unrelated inventory list.
- **Every keybind restored to the position table, plus a new Joystick keybinds table.** The table-conversion above initially dropped the Start/Coin/admin key letters and both joysticks' 4 direction keys, since a joystick's 4 keys don't fit one grid cell the way a single button's key does. Start/Coin/admin cells now carry their key letters inline (matching the button cells' existing convention); a new `##### Joystick keybinds` table gives Player 1's arrow keys and Player 2's letter keys (`N`/`Q`/`M`/`O`) side by side.
- **Fixed a pre-existing internal inconsistency: pin42 was documented as both "Mouse Middle Button" (Pin-to-key mapping table) and "Right Mouse Button" (Button function reference table) for the same physical button.** Not a real hardware ambiguity — the second table already agreed with `LEDBlinkyInputMap.xml`'s `RMOUSE` port label and the panel photo; the first table had a stale typo that went unnoticed until the new position table pulled from it and surfaced the contradiction. Pin-to-key mapping corrected to "Left Click"/"Right Click" (aligned to the user's own terms) to match.
- **Pin-to-key mapping confirmed correct by direct button-press testing** — that part held up. **The rest of this entry was wrong and is retracted; see the port-label fix further down.** A discrepancy surfaced earlier this PR (`LEDBlinkyInputMap.xml`'s `inputCodes` disagreeing with the board:port table on Buttons 4-6, both players) was at the time judged resolved in the table's favor, on the theory that `inputCodes` was stale, harmless Animation-Editor-only metadata nothing depended on. Real-hardware calibration testing later proved the opposite: `inputCodes` was right, the table's board:port assignments were wrong.

### Fixed

- **`LEDBlinkyInputMap.xml`'s PAC-LED64 port `label` attribute was wrong for `P1B4`/`P1B5`/`P1B6`/`P1B7` and `P2B4`/`P2B5`/`P2B6`/`P2B7` — a 4-way rotation on each board (each label's LED was really wired to the *next* button's port: `P1B4`→`P1B5`'s port, `P1B5`→`P1B6`'s, `P1B6`→`P1B7`'s, `P1B7`→`P1B4`'s; same pattern on board 2).** Confirmed on the cabinet via a one-at-a-time calibration `.lwax` (every LED-equipped control lit alone in sequence) after the P1START-merge fix still showed a rotated lighting order isolated to exactly these 8 controls. This also re-explains and corrects an earlier conclusion in this same PR: `LEDBlinkyInputMap.xml`'s `inputCodes` attribute (e.g. `KEYCODE_D` recorded on the port labeled `P1B4`) was previously judged to be the wrong/stale field and dismissed as harmless, optional Animation-Editor-only metadata — that was backwards. `inputCodes` was correctly reporting which button each port really lights; the port's `label` — the field every `.lwax` animation actually addresses by — was the one that was wrong. Fixed by correcting the 8 affected ports' `label` attributes (no `inputCodes` values needed to change, they already agreed with the corrected labels) and updating the Master control reference table's board:port column to match. All animations regenerated against the corrected map — this affects the addressing used by every generated file, not just the 9 effects that needed regeneration for the earlier two fixes. **Confirmed resolved on the actual cabinet**: the corrected `LEDBlinkyInputMap.xml` was installed in place of the original and every control now lights in its correct position. Since this is the same file LedBlinky's runtime uses to route per-ROM `Colors.ini` colors to physical ports, not just something `.lwax` animations read, this should also correct any wrong in-game LED color that predates this PR on Buttons 4-7.
- **`P1START` folded into the `P1COIN`/`P1B1`/`P1B5` group in `LEFT_RIGHT_ORDER`** (the lwax-animation skill's cabinet layout data), by explicit request after testing the bottom-row fix — the user didn't want P1START lighting as an isolated single-control blip at the far end of every sweep/pulse/race/fill effect. This is a deliberate animation-grouping override, not a physical-position correction: the Physical button layout table is unchanged (P1START really does sit in its own column above P1's joystick). Side benefit: `RADIAL_RINGS` is now perfectly symmetric (7 groups either side of the trackball, was 8-vs-7). Same 9 effects regenerated as the bottom-row fix below (32 files total).
- **Doc audit: 4 small inconsistencies found and fixed.** `cli-cheatsheet.md`'s LEDBlinky section never got a `lwax fade` entry when the command was added — added one alongside the existing Step-numbered commands, matching the flags actually implemented in `cli.py`/documented in `commands.md`. The Mini-PAC/PAC-LED64 hardware note pointed readers to the `.lwax` section for a "full per-board RGB channel map" that section no longer contains (it was simplified to point at the Master control reference during the consolidation) — repointed directly. `.claude/skills/lwax-animation/SKILL.md`'s test-coverage note listed only the original 5 builders, missing `build_drain`/`build_alternate` added later. The generated-animation count was off by one (68 → 67) in both `CHANGELOG.md` and the PR body — `rainbow_scroll` contributes 3 speed-variant files with no separate base file, which the original arithmetic double-counted. Also dropped a stray leftover `</p>` HTML tag at the end of an unrelated `fill-defaults --override-uniform` bullet.
- **This cabinet's `LEFT_RIGHT_ORDER` reference data (in the lwax-animation skill) never included the bottom row (`P1B5-8`/`P2B5-8`) at all**, so every effect derived from it or from `RADIAL_RINGS` (sweeps left/right, both radial pulses, the breathing pulse, the P1-vs-P2 race, the rainbow scroll, the combo-meter fill, the countdown/fuse) silently skipped 8 of the 27 controls. Confirmed on the cabinet: `pulse_outward_from_trackball_toxic.lwax` never lit Buttons 5-8. Fixed by folding each bottom-row button into its top-row column-mate's group; confirmed via hardware testing afterward that ring/column *spacing* was already correct as index-based (not compressed for the admin row's tighter physical packing). All 32 affected generated files regenerated (9 base effects + their Fire/Ice/Toxic variants where applicable). `ROWS`/`CYCLONE_LOOP`/`RAIN_DROP_GROUPS`-based effects were never affected and didn't need touching.
...
- **GUI: "Toolkit" tab renamed to "Custom Wheels".** The tab has only ever covered the three synthetic wheels (Favorites / Recently Played / Most Played) — building, registering, managing, install-helper wiring, auto-refresh scheduling, and scrub — so "Toolkit" (a stylistic rename from the original "Tools" in v2.6.0, without a scope change) read as a vaguer, more general-purpose name than the tab's actual contents. Every GUI string, doc, and README reference updated to match; the CLI's `spindoctor install-tools --add-to-system <NAME>` examples that use `Toolkit` as an example *HyperSpin wheel name* (an unrelated, pre-existing naming convention some cabinets use for a maintenance-tasks wheel) are unchanged.
...
- **Refreshed the Favorites and Most Played attract-mode videos and active-browsing music.** New source video (High Profile, B-frames enabled) was re-encoded to Main Profile / Level 4.0 / `-bf 0` per the documented Windows 7 Adobe AIR requirement before bundling; new source songs (WAV) were converted to MP3 at 192 kb/s to match the existing active-browsing music format. Background images are unchanged, so `bg_Favorites.png` / `bg_Most_Played.png` were left as-is. Prior bundled video/music preserved in `spindoctor/assets/archive/` (existing `video_*.mp4` archive copies rolled to `video_*_old.mp4`).
...
- **Synthetic-wheel attract videos (Favorites, Most Played, Recently Played, Recompiled) started playing 1-2s late, or dropped video entirely, after the recent media refresh — two separate causes.** (1) All four bundled `video_*.mp4` assets were re-encoded at H.264 **High Profile**, and two of them (Favorites, Recompiled, sourced at 2752×1536) at **Level 5.0** — both violate the Windows 7 Adobe AIR runtime's documented ceiling of **Main Profile, Level 4.0** (`docs/synthetic-wheel-media.md`), which silently drops the video track (audio keeps playing) on that runtime. (2) Independently, all four were encoded with libx264's default 3 B-frames per GOP; at these videos' low 2 fps, a decoder's reorder buffer alone accounts for ~1.5s before it can display frame 0 — reproducing the exact "audio starts immediately, video is late" symptom even once the profile/level are fixed. All four assets are re-encoded to Main Profile / Level 4.0 / ≤1920×1080 / **0 B-frames** (`-bf 0`, plus `-movflags +faststart`), same duration and audio, matching the format that has always worked on tested cabinets. Also fixed two stale doc/comment references to the old "2× music duration" / ~58-61s runtime — the current bundled videos are 120s (4× the 30s music track).
- Added `test_bundled_video_is_windows7_compatible_h264` (`tests/test_bundled_assets.py`), which ffprobes each of the four bundled videos and pins profile/level/resolution/`has_b_frames` against the documented Windows 7 requirement, so a future media refresh can't silently regress either issue again. Skipped when `ffprobe` isn't on `PATH` (not a CI dependency).
...
- **Ultimarc Mini-PAC + PAC-LED64 hardware documented.** New section in `cabinet-architecture-reference.md` covering: board identification (Mini-PAC = keyboard encoder, PAC-LED64 = LED controller); WinIPAC as the EEPROM configuration tool (note: button nicknames live in the XML project file, not the EEPROM); physical button layout diagram with swop-key combos in the footer; full pin-to-key mapping for all 34 input pins decoded from the WinIPAC XML export; swop (secondary) key functions (P1 Coin + Joystick Up/Down = Windows Volume Up/Down via consumer HID, P1 Coin + Select = Tab = RetroArch Quick Menu); and per-player button tables with Mini-PAC pin, key, function, and RetroArch action columns ordered consistently as pin | key | function.
...
- **`find-orphan-media --verbose` showed full file paths in `--apply` mode but not in dry-run mode.** The table already printed relative paths for all orphans; adding `--verbose` in dry-run now also prints each absolute path prefixed with `would delete:`, matching the apply-mode output.

- **`add-pc-system` dry-run never warned about stale PCLauncher INIs that would be deleted unless `--verbose` was also passed.** With `--apply`, stale INI deletions were always reported. Without `--apply`, the same warning only appeared under `--verbose`, so users could run a plain dry-run, see no mention of deletions, then be surprised when `--apply` deleted INIs. The stale-deletion preview now shows in all dry-run modes (verbose and non-verbose alike).

- **`add-pc-system --apply --verbose` did not list individual game names added to the database.** In dry-run + verbose mode the command printed each game title as `new` or `existing`; with `--apply` it only printed the summary line `X stub(s) added`. The apply path now collects newly-added titles into a list and, when `--verbose` is set, prints each one after the summary line — matching the dry-run output.
...
- **`pc-fix-exe` updated the wrong file when both a system-level and per-game PCLauncher INI exist.** The command preferred `Modules/PCLauncher/<System>.ini` (system-level) over `Modules/PCLauncher/<System>/<game>.ini` (per-game). For standard PC systems set up by `add-pc-system`, PCLauncher reads the per-game INI, so fixing the system-level file had no effect on the game that was actually launched. The priority is now inverted: per-game INI wins when it exists; system-level is the fallback for Taito Type X / NESiCAxLive systems that have no per-game subfolder; when neither exists, a new per-game INI is created.
...
- **GUI status bar: long command strings no longer hide checkboxes.** When a command with many flags (e.g. `add-pc-system --no-menu --no-system-media --no-game-media --verbose --apply`) was running, the status bar label demanded enough horizontal space to display the full command string, pushing the **Apply / Verbose / Save Log** checkboxes and the **Hide / Copy / Clear / Stop** buttons off screen. The label now has a fixed minimum width of 1 character so the right-side controls always stay visible. `_set_status` dynamically truncates any text that would overflow the allocated label width, appending … so the truncation is visually obvious.
...
- **Tests for PC scan junk/web-URL filtering:** Three new tests in `test_recursive_scan.py` pin the behaviour introduced in PR #362 — junk executables (`setup.exe`, `vcredist_x64.exe`) inside a game folder are deprioritised in favour of the real game exe; root-level `.url` files whose `URL=` line is an `http(s)://` address are dropped; root-level `.lnk`/`.url` files whose stem starts with `"Launch "` are dropped. Previously untested, so a regression would have gone unnoticed.
- **`game save-order` command:** New subcommand that saves a custom game order to a system's wheel XML. Accepts `--order-file <path>` (one ROM name per line) to avoid Windows 7's ~32 KB command-line limit on large systems. Games absent from the file are appended after the listed entries so nothing is dropped. The GUI **Games** tab → **Save Order** button now shells out to this command instead of calling library functions directly, fixing a GUI/CLI parity violation.
