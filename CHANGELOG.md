...
- **"What's new" dialog now fires for pre-2.0 upgraders.** A long-time 1.x user whose existing config has no `last_seen_version` field was being treated as a fresh install (suppressed and silently stamped), so they would *never* see the dialog the feature exists to surface. The check now distinguishes fresh installs (`first_run_complete=False`) from upgraders (`first_run_complete=True && last_seen_version==""`) and shows the dialog for the latter on first launch after upgrading.
...
- **2.0-readiness doc refresh.** Every audit gap closed: `docs/configuration.md` now lists the `last_seen_version` config key and the `SPINDOCTOR_DISABLE_SINGLETON` env var; `docs/gui.md` documents the **Help → What's new** and **Help → Keyboard shortcuts** dialogs, the single-instance lock, and the Ctrl+C / Stop safe-interrupt semantics, and the keyboard-shortcut table now covers `Ctrl+=` and `Esc`; `docs/migrating-from-1.x.md` lists the fourth new config key plus the six new GUI affordances (singleton lock, What's-new dialog, Keyboard-shortcuts dialog, Ctrl+C safe interrupt, `--skip-ambiguous` wiring, the upgrader path for What's-new); `docs/commands.md` documents `fetch-meta --skip-ambiguous` with a full three-way override table (`--auto-best` / `--skip-ambiguous` / `--interactive`) and a top-of-doc note on safe interrupts; `docs/troubleshooting.md` adds entries for the Win 7 TLS-handshake symptom, the "second GUI won't open" by-design behaviour, and the post-crash `gui.lock` file; `README.md` line 84 surfaces the new Help-menu dialogs, the About-dialog icon, and the singleton lock alongside the existing GUI feature paragraph. No semantic changes to existing docs — additive only.
