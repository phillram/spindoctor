...
- **`spindoctor.lwax.build_wave()`, `build_rain()`, `build_rainbow_scroll()`, `build_fill()`, and `merge_animations()`** — generic animation primitives alongside `build_color_cycle()`, used to generate this cabinet's 16 requested `.lwax` animations (directional sweeps, rainfall, radial pulses, a ping-pong "breathing" pulse, confetti, a two-color race from a shared center, a scrolling rainbow, a Cylon-style scanner, clockwise/counter-clockwise "cyclone" racetrack loops, an accumulating combo-meter fill, and a heartbeat brightness pulse). `build_wave()` moves a leading color (with an optional trailing second color) across an ordered list of control-label *groups*, wrapping continuously — the same function covers sweeps, radial pulses, ping-pong scanners (double the group list as `groups + groups[-2:0:-1]`), and racetrack "cyclone" loops (concatenate two parallel rows, one reversed); direction is just the order groups are passed in. A group can hold more than one label when two controls are lit at the same physical spot (e.g. a Start button sharing an action button's position) — `build_rainbow_scroll()` takes the same grouped-position convention (not a flat label list) so co-located labels always get the identical hue. `build_rain()` runs independent, randomly-timed drop sequences per group (one-label-per-group turns it into confetti instead of rain). `build_fill()` accumulates lit groups instead of moving a single position, holds, flashes, and loops. `merge_animations()` layers independently-built, frame-synchronized animations into one (e.g. two waves racing from a shared center in different colors, using mismatched group-list lengths on each side by crossing their `frames_per_step` values so both land on the same total frame count). Not exposed as CLI subcommands — the column/row/ring/loop groupings are cabinet-specific data, generated via a one-off script rather than new CLI flags, per user preference to avoid growing the CLI surface for effects that are mostly about picking colors/timings once.
