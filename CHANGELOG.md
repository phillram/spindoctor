...
- **HyperSpin theme zips for synthetic wheels** — SpinDoctor now bundles and installs `Media\Main Menu\Themes\<SystemName>.zip` for `Favorites`, `Most Played`, and `Recently Played`. Without a theme zip HyperSpin may not display the attract-mode video at all for the system. Each zip contains a `Theme.xml` with a centred 16:9 `<video>` element (600×338 on HyperSpin's 1024×768 canvas), minimal blank FWS SWF placeholders (`Background.swf`, `Artwork1.swf`, `Artwork4.swf`), a `Video.png` thumbnail scaled from the bundled background image, and an `Info.txt`. Installed by `rebuild --apply` (skip-if-exists) and `mainmenu add --apply` (always overwrite). Theme zips are added to the `pyproject.toml` package-data glob (`assets/*.zip`).
