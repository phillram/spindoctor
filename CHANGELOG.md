...
- **`scripts/generate_lwax_patterns.py`** — one-off generator that emits a colour-diverse batch of 35 LEDBlinky `.lwax` animations (7 effect families × 5 variants: fade, sweep/chase, rain/confetti, rainbow-scroll, fill, drain, checkerboard) into `~/Downloads/spindoctor-lwax-patterns/`. Every fixed-colour variant uses a globally unique colour (hue-strided so families don't clump), each family includes one moving/fading rainbow variant, and each spreads over slow/medium/fast timing. Builds on the `spindoctor.lwax` builders plus local rainbow helpers; files are raw/unsigned and still need the Animation Editor Save-As signing step on the cabinet.
