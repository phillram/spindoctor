...
- **`generate-config` — no longer wipes `Emu_Path` from per-system `Emulators.ini` after a ROM drive migration** — `generate_rl_system_ini` now reads the existing `Emu_Path` value from the `[<Emulator>]` section before overwriting the file and writes it back into the new file. Previously the entire file was replaced with a template that only contained `Rom_Path=`, silently dropping the emulator executable path that HyperHQ / RLUI had written. RocketLauncher then fell back to `Global Emulators.ini`; if that file was also generated by SpinDoctor it used the wrong key name (see below), producing "Could not find an Emu_path for RetroArch" for every game on every console.
- **`generate-config` — `Global Emulators.ini` now uses `Emu_Path` / `Rom_Extension` (the keys RocketLauncher actually reads)** — `generate_global_emulators_ini` previously wrote `Emulator_Application_Path=` and `Emulator_Extension=`, which are not recognised by RocketLauncher. Any `Global Emulators.ini` created by SpinDoctor was effectively empty from RL's perspective; when a per-system `Emulators.ini` also lacked `Emu_Path` (see above), every launch failed with "Could not find an Emu_path" regardless of drive letter.
