...
- **Daphne Singe (WoW Action Max) documented in cabinet architecture reference.** Covers the custom Daphne Singe 1.0.10 build layout, four failure modes in launch order with distinct symptoms and fixes, and the critical D:/J: asset split: per-game `.singe` scripts and video files live on J:, but all engine assets (Emulator.singe, sprites, sounds, fonts) must live in `singe\ActionMax\` on D: because `Emulator.singe` loads them via `singe/ActionMax/<file>` paths relative to daphne.exe's working directory — they cannot be redirected to J: without editing Emulator.singe. `SingePathUpdate` in `Daphne Singe.ini` is not useful for WoW Action Max and should be left off.
