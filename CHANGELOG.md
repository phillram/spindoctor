...
- **`scripts/generate_lwax_patterns.py`** — one-off generator that emits a colour-diverse batch of LEDBlinky `.lwax` animations into `~/Downloads/spindoctor-lwax-patterns/`. 15 effect families × 5 variants (fade, sweep, rain/confetti, scroll, fill, drain, checker, radial pulse, cyclone, race-from-centre, heartbeat, strobe, marquee, VU-meter bounce, row-blink), each with one moving/fading rainbow variant and a slow/medium/fast spread, plus a `breathe_*` solid-colour library (whole panel fading one colour in and out) in standard, `vivid_*`, and `pastel_*` sets with a `breathe_rainbow`. Every fixed-colour variant uses a globally unique, hue-strided colour so families don't clump. Builds on the `spindoctor.lwax` builders plus local rainbow/pulse helpers; files are raw/unsigned and still need the Animation Editor Save-As signing step on the cabinet.
