...
# for these when called without --apply):
#   generate-config  find-misplaced  find-orphan-media
#   doctor           lightgun detect lightgun configure
#
# Note: "mainmenu show" with NO system argument is genuinely read-only (it
# only renders a numbered table).  The write variant "mainmenu show SYSTEM
# --apply" unhides a system but is never generated by the GUI's own buttons;
# it appears only if the user types it manually in Custom Command.  The
# no-arg form is the dominant case, so it stays in this set.
