...
class _SpinDoctorGUI:
    ...
    def _build_backup_tab(self, parent):
        ...
        # ── Scrub cabinet data ────────────────────────────────────────────────
        scrub_outer = self.ttk.LabelFrame(frame, text="Scrub cabinet data")
        scrub_outer.grid(row=7, column=0, columnspan=4, sticky="ew", pady=(12, 4))
        scrub_outer.columnconfigure(1, weight=1)

        self.ttk.Label(
            scrub_outer,
            text=(
                "Permanently reset favorites and/or play statistics. "
                "Use 'Backup first' to create a recoverable copy before deleting — "
                "Statistics.ini files cannot be regenerated by SpinDoctor."
            ),
            wraplength=800, justify="left",
        ).grid(row=0, column=0, columnspan=4, sticky="w", padx=6, pady=(4, 6))

        # What to scrub
        scrub_what_frame = self.ttk.Frame(scrub_outer)
        scrub_what_frame.grid(row=1, column=0, columnspan=4, sticky="w", padx=6, pady=2)
        self.ttk.Label(scrub_what_frame, text="What to clear:").pack(side="left")
        ...
        _scrub_fav_chk = self.ttk.Checkbutton(
            scrub_what_frame, text="Favorites", variable=self._scrub_favorites_var,
        )
        ...
        _attach_tooltip(
            ...
            "The favorites store can be repopulated with 'fav add' or 'fav sync'.",
            self.tk,
        )
        _scrub_stats_chk = self.ttk.Checkbutton(
            scrub_what_frame, text="Play statistics", variable=self._scrub_stats_var,
        )
        ...
        _attach_tooltip(
            ...
            "WARNING: Statistics.ini files are written by RocketLauncher and\n"
            "cannot be regenerated — use 'Backup first' below before scrubbing.",
            self.tk,
        )

        # Optional backup directory
        scrub_bk_row = self.ttk.Frame(scrub_outer)
        scrub_bk_row.grid(row=2, column=0, columnspan=4, sticky="ew", padx=6, pady=(4, 2))
        self.ttk.Label(scrub_bk_row, text="Backup first to:").pack(side="left")
        ...
        self.ttk.Label(
            scrub_outer,
            text="(Leave blank to skip backup — not recommended for play statistics)",
            foreground="#888888",
        ).grid(row=3, column=0, columnspan=4, sticky="w", padx=6, pady=(0, 4))

        # Apply + run
        scrub_btn_row = self.ttk.Frame(scrub_outer)
        scrub_btn_row.grid(row=4, column=0, columnspan=4, sticky="w", padx=6, pady=(2, 6))
        ...
        # Restore section
        scrub_restore_frame = self.ttk.LabelFrame(scrub_outer, text="Restore from scrub backup")
        scrub_restore_frame.grid(
            row=5, column=0, columnspan=4, sticky="ew", padx=6, pady=(8, 6),
        )
        scrub_restore_frame.columnconfigure(1, weight=1)

        self.ttk.Label(scrub_restore_frame, text="Backup folder").grid(
            row=0, column=0, sticky="w", padx=6, pady=2,
        )
        self._scrub_restore_path_var = self.tk.StringVar()
        self.ttk.Entry(
            scrub_restore_frame, textvariable=self._scrub_restore_path_var, width=50,
        ).grid(row=0, column=1, sticky="ew", padx=6, pady=2)
        self.ttk.Button(
            scrub_restore_frame, text="Browse…",
            command=lambda: self._browse_backup_dir(
        ...
        scrub_restore_btn_row = self.ttk.Frame(scrub_restore_frame)
        scrub_restore_btn_row.grid(
            row=1, column=0, columnspan=3, sticky="w", padx=6, pady=(2, 6),
        )
