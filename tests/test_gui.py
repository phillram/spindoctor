...
def test_whats_new_dialog_constructs(monkeypatch):
    app, tk_mod = _build_gui_for_test(monkeypatch)
    try:
        before = set(app.root.winfo_children())
        app._show_whats_new()
        ...
        titles = [w.title() for w in new_windows]
        assert any("What's new" in t for t in titles), titles
        for w in new_windows:
            w.destroy()
    finally:
        app.root.destroy()
...
def test_maybe_show_whats_new_suppressed_on_fresh_install(monkeypatch, tmp_path):
    """A blank ``last_seen_version`` means fresh install — the first-run
    wizard owns the conversation; "What's new" must not pile on."""
    ...
    try:
        ...
        monkeypatch.setattr(gui_mod, "save_config", lambda _cfg: None)

        app._maybe_show_whats_new()
        assert called == [], "fresh install should not see What's new"
    finally:
        app.root.destroy()


def test_maybe_show_whats_new_fires_on_version_change(monkeypatch):
    """An old recorded version triggers the dialog and re-stamps config."""
    from spindoctor import gui as gui_mod
    ...
    try:
        ...
        class _Cfg:
            last_seen_version = "1.0.0-ancient"
        ...
        monkeypatch.setattr(gui_mod, "load_config", lambda: cfg)
        monkeypatch.setattr(gui_mod, "save_config", fake_save)

        app._maybe_show_whats_new()
        assert called == [True], "expected dialog to fire on version mismatch"
        # Config should now be stamped with the running version so we
        # don't pester the user on every launch.
